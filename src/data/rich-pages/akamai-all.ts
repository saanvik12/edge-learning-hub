...
export const akamaiPages: Record<string, RichPageData> = {
  "akamai-dns": {
    ...
    hero: {
      ...
      mentalModel: "DNS is the internet's phone book. When a user types your domain, their resolver queries authoritative nameservers for the IP address. With Akamai Edge DNS, those nameservers are distributed across 1000+ PoPs globally, answering queries from the nearest location. This means faster resolution, built-in DDoS protection, and 100% SLA uptime.",
    },
    architecture: {
      sectionLabel: "DNS Resolution",
      title: "How a DNS Query Resolves",
      description: "The complete path from user's browser to your origin server, via recursive and authoritative DNS.",
      pipeline: [
        { label: "User Browser", description: "Types 'www.example.com'", badgeColor: "muted" },
        { label: "Recursive Resolver", description: "ISP/Google/Cloudflare resolver checks cache", badgeColor: "muted" },
        { label: "Root → TLD NS", description: "Directed to .com TLD nameservers", badgeColor: "muted" },
        { label: "Edge DNS", description: "Akamai authoritative NS returns A/AAAA record", badge: "Akamai", badgeColor: "accent" },
        { label: "IP Returned", description: "Browser connects to the resolved IP address", badgeColor: "muted" },
      ],
    },
    concepts: [
      { title: "Record Types", content: "A records map domains to IPv4 addresses. AAAA records map to IPv6. CNAME records alias one domain to another (www → example.com). MX records direct email. TXT records hold metadata (SPF, DKIM, domain verification). NS records delegate authority. SOA records define zone parameters (refresh intervals, TTLs). SRV records specify service locations." },
      { title: "TTL (Time to Live)", content: "TTL tells resolvers how long to cache a DNS answer. Short TTL (60-300s): fast failover, more queries, slower for users. Long TTL (3600-86400s): faster resolution, fewer queries, slow propagation on changes. Best practice: use long TTLs for stable records, short TTLs before planned changes, and restore long TTLs after changes propagate." },
      { title: "Authoritative vs. Recursive DNS", content: "Authoritative DNS servers are the 'source of truth' for a zone — they hold the actual records. Recursive resolvers (like 8.8.8.8) query authoritative servers on behalf of clients and cache results. Akamai Edge DNS is an authoritative DNS service — it's where your zone records live and are served from." },
      { title: "DNSSEC", content: "DNS Security Extensions (DNSSEC) adds cryptographic signatures to DNS records, preventing spoofing and cache poisoning. The chain of trust goes: root zone → TLD → your zone. Each level signs the next level's keys. Akamai Edge DNS supports DNSSEC with automated key rotation — you just enable it." },
      { title: "Zone Apex (Naked Domain)", content: "The zone apex (example.com without 'www') cannot use a CNAME per RFC standards. Options: use an A record pointing directly to Akamai IPs, or use Akamai's ALIAS/ANAME record (a non-standard extension that resolves a CNAME at query time and returns an A record to the client)." },
    ],
    comparisons: [
      {
